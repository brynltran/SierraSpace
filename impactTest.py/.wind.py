...
RPM_MAX = 100  # Adjust -- 320 is gussed RPM 
RPM_CEIL = 1
def setup_hx711():
...
def stepper_motor_controller(conn, motor_speed_shared):
    setup_stepper()
    pid = PID(0.50, 0.5, 2.3, setpoint=500)  # Adjust these PID values based on testing
...
def main():
    ...
    try:
        ...
        def update(frame):
            ...
            sheet1.write(itx, 0, float(current_time))
            sheet1.write(itx, 1, float(tension_value))
            sheet1.write(itx, 2, float(motor_speed_shared.value))
            itx += 1
        ...
        print("Interrupted by user, stopping motor and saving data.")
    finally:
        wb.save('/home/brtr7527/sierraspace/feedback/data/test.xls')
