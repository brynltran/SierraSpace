...
RPM_MAX = 520  # Adjust -- 320 is gussed RPM 
...
def stepper_motor_controller(conn, motor_speed_shared):
    setup_stepper()
    pid = PID(0.01, 0.1, 0.05, setpoint=2)  # Adjust these PID values based on testing
    ...
    while True:
        ...
        if conn.poll():
            ...
            rpm = (pid_output / 2.0) * RPM_MAX  # Adjust the PID output to RPM scaling -- Maybe lower RPM? or change PID
        ...
        if current_time - last_step_time >= step_interval:
            GPIO.output(DIR_PIN, GPIO.LOW if rpm >= 0 else GPIO.HIGH)
